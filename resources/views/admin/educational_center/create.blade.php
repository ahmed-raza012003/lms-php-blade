    ...
    <div class="main-content">
        <section class="section">
            <div class="section-header">
                <h1>{{ __('Create Educational Center') }}</h1>
            ...
            <div class="section-body">
                ...
                <div class="card">
                    <div class="card-header">
                        <h4>{{ __('Add New Educational Center') }}</h4>
                    </div>
                    <div class="card-body">
                        <form action="{{ route('admin.educenter.store') }}" method="POST">
                            @csrf
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="operating_hours" id="operating_hours" class="form-control @error('operating_hours') is-invalid @enderror" value="{{ old('operating_hours') }}">
                            ...
                            <div class="form-group">
                                ...
                                <select name="verification_status" id="verification_status" class="form-control @error('verification_status') is-invalid @enderror">
                                    <option value="pending" {{ old('verification_status') == 'pending' ? 'selected' : '' }}>{{ __('Pending') }}</option>
                                    <option value="approved" {{ old('verification_status') == 'approved' ? 'selected' : '' }}>{{ __('Approved') }}</option>
                                    <option value="rejected" {{ old('verification_status') == 'rejected' ? 'selected' : '' }}>{{ __('Rejected') }}</option>
                                </select>
