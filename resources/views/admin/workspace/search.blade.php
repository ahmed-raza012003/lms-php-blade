...
@section('contents')
    <!-- Breadcrumb -->
    <x-frontend.breadcrumb :title="__('Workspaces')" :links="[['url' => route('home'), 'text' => __('Home')], ['url' => '', 'text' => __('Workspaces')]]" />

    <!-- Workspaces Section -->
    <section class="workspaces-area" style="padding: 80px 0; background: linear-gradient(to bottom, #f8f9fa, #ffffff);">
        <div class="container">
            <div class="row">
                <!-- Main Content -->
                <div class="col-xl-9 col-lg-8">
                    <!-- Search Form -->
                    <div class="workspaces-top-wrap" style="margin-bottom: 30px;">
                        <div class="tgmenu__search d-none d-md-block" style="max-width: 500px; margin: 0 auto;">
                            <form action="{{ route('search_workspaces') }}" class="tgmenu__search-form" style="display: flex; align-items: center; background: #fff; border-radius: 50px; box-shadow: 0 4px 15px rgba(0,0,0,0.1); overflow: hidden;">
                                ...
                                <div class="input-grp" style="flex: 1; display: flex;">
                                    <input type="text" placeholder="{{ __('Search For Workspaces') }} . . ." name="search" value="{{ request('search') }}" style="border: none; padding: 12px 15px; width: 100%; font-size: 16px; outline: none;">
                    ...
                    <!-- Workspaces Grid -->
                    <div class="row workspaces__grid-wrap row-cols-1 row-cols-xl-3 row-cols-lg-2 row-cols-md-2 row-cols-sm-1">
                        @forelse($workspaces as $workspace)
                            <div class="col" style="margin-bottom: 30px;">
                                <div class="workspaces__item card" style="background: #fff; border: none; border-radius: 15px; box-shadow: 0 8px 25px rgba(0,0,0,0.1); overflow: hidden; transition: transform 0.3s, box-shadow 0.3s; position: relative;">
                                    <div style="width: 100%; height: 200px; background: #f0f4f8; display: flex; justify-content: center; align-items: center; overflow: hidden;">
                                        @if($workspace->profile_photo)
                                            <img src="{{ $workspace->profile_photo }}" alt="{{ $workspace->name }}" style="width: 100%; height: 100%; object-fit: cover;">
                                        @else
                                            <i class="fas fa-square" style="font-size: 80px; color: #6b7280;"></i>
                                        @endif
                                    </div>
                                    <div class="card-body" style="padding: 20px;">
                                        <h5 class="card-title" style="font-size: 20px; font-weight: 600; color: #1f2937; margin-bottom: 15px; text-align: center; background: linear-gradient(to right, #3b82f6, #10b981); -webkit-background-clip: text; -webkit-text-fill-color: transparent;">{{ $workspace->name }}</h5>
                                        <p class="card-text" style="font-size: 14px; color: #4b5563; margin-bottom: 10px;"><strong style="color: #1f2937;">Type:</strong> {{ $workspace->workspace_type }}</p>
                                        <p class="card-text" style="font-size: 14px; color: #4b5563; margin-bottom: 10px;"><strong style="color: #1f2937;">Description:</strong> {{ Str::limit($workspace->description ?? 'No description provided', 100) }}</p>
                                        <p class="card-text" style="font-size: 14px; color: #4b5563; margin-bottom: 10px;"><strong style="color: #1f2937;">Location:</strong> {{ $workspace->location ?? 'Not specified' }}</p>
                                        <p class="card-text" style="font-size: 14px; color: #4b5563; margin-bottom: 10px;"><strong style="color: #1f2937;">Address:</strong> {{ $workspace->address ?? 'Not specified' }}</p>
                                        <p class="card-text" style="font-size: 14px; color: #4b5563; margin-bottom: 10px;"><strong style="color: #1f2937;">City:</strong> {{ $workspace->city ?? 'Not specified' }}</p>
                                        <p class="card-text" style="font-size: 14px; color: #4b5563; margin-bottom: 10px;"><strong style="color: #1f2937;">Country:</strong> {{ $workspace->country ?? 'Not specified' }}</p>
                                        <p class="card-text" style="font-size: 14px; color: #4b5563; margin-bottom: 20px;"><strong style="color: #1f2937;">Rating:</strong> {{ $workspace->rating ? number_format($workspace->rating, 2) . '/5' : 'Not rated' }}</p>
                                        <button class="btn btn-secondary select-workspace" disabled style="display: block; width: 100%; padding: 12px; background: linear-gradient(to right, #6b7280, #4b5563); border: none; border-radius: 10px; color: #fff; font-size: 16px; font-weight: 500; cursor: not-allowed; transition: background 0.3s;">Select Workspace</button>
                                    </div>
                        ...
                        @empty
                            <div class="col" style="text-align: center; padding: 50px 0;">
                                <p style="font-size: 18px; color: #6b7280;">No workspaces available.</p>
                            </div>
                        @endforelse
                    </div>
