...
@section('title')
    <title>{{ __('Edit Room Schedule') }}</title>
@endsection
            ...
            <div class="section-body">
                ...
                <div class="card">
                    <div class="card-header">
                        <h4>{{ __('Edit Schedule') }}</h4>
                    </div>
                    <div class="card-body">
                        <form action="{{ route('admin.room_schedules.update', $schedule->schedule_id) }}" method="POST">
                            @csrf
                            @method('PUT')
                            <div class="form-group">
                                ...
                                <select name="room_id" class="form-control">
                                    ...
                                    @foreach ($rooms as $room)
                                        <option value="{{ $room->room_id }}" {{ old('room_id', $schedule->room_id) == $room->room_id ? 'selected' : '' }}>{{ $room->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                <select name="is_available" class="form-control">
                                    <option value="1" {{ old('is_available', $schedule->is_available) == '1' ? 'selected' : '' }}>{{ __('Available') }}</option>
                                    <option value="0" {{ old('is_available', $schedule->is_available) == '0' ? 'selected' : '' }}>{{ __('Unavailable') }}</option>
                                </select>
                            </div>
