            ...
            <div class="section-body">
                ...
                <div class="card">
                    <div class="card-header">
                        <h4>{{ __('Update Assistant Assignment') }}</h4>
                    </div>
                    <div class="card-body">
                        <form action="{{ route('admin.assistants.update', $assistant->assistant_id) }}" method="POST" id="assistantForm">
                            @csrf
                            @method('PUT')
                            <div class="form-group">
                                ...
                                <select name="assistant_user_id" id="assistant_user_id" class="form-control @error('assistant_user_id') is-invalid @enderror" required>
                                    ...
                                    @foreach ($users as $user)
                                        <option value="{{ $user->id }}" {{ $assistant->assistant_user_id == $user->id ? 'selected' : '' }}>{{ $user->name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <select name="user_id" id="user_id" class="form-control @error('user_id') is-invalid @enderror" required>
                                    ...
                                    @foreach ($users as $user)
                                        <option value="{{ $user->id }}" {{ $assistant->user_id == $user->id ? 'selected' : '' }}>{{ $user->name }}</option>
                                    @endforeach
                                </select>
                                @error('user_id')
                                    <div class="invalid-feedback">{{ $message }}</div>
                            ...
                            <div class="form-group">
                                ...
                                <select name="authority_level" id="authority_level" class="form-control @error('authority_level') is-invalid @enderror" required>
                                    <option value="limited" {{ $assistant->authority_level == 'limited' ? 'selected' : '' }}>{{ __('Limited') }}</option>
                                    <option value="full" {{ $assistant->authority_level == 'full' ? 'selected' : '' }}>{{ __('Full') }}</option>
                                </select>
